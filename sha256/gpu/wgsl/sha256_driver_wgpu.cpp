// WebGPU/WGSL host driver for batched SHA-256 (FIPS 180-4).
//
// Mirrors the Metal/CUDA layout: caller supplies a packed input arena, an
// (offset, length) descriptor per input, and a contiguous 32-byte stride
// output buffer.
//
// Build flags:
//   * KINET_SHA256_HAS_WEBGPU=1      - Dawn or wgpu-native runtime found
//   * KINET_SHA256_HAS_WGPU_NATIVE=1 - wgpu-native specifically (gives
//                                    wgpuDevicePoll for synchronous waits)

#include "sha256_driver_wgpu.h"

#if defined(KINET_SHA256_HAS_WEBGPU)

#include <webgpu.h>
#if defined(KINET_SHA256_HAS_WGPU_NATIVE)
#  include <wgpu.h>
#endif
...
bool init_engine() {
    ...
    wgsl.code = sv(kSHA256_WGSL_Source);
    ...
    if (!e.module) {
        std::fprintf(stderr, "wgpu: sha256 shader compile failed\n");
        return false;
    }

    WGPUComputePipelineDescriptor cpd{};
    cpd.compute.module = e.module;
    cpd.compute.entryPoint = sv("sha256_jobs");
    cpd.label = sv("sha256_jobs");
    e.pipeline = wgpuDeviceCreateComputePipeline(e.device, &cpd);
    if (!e.pipeline) {
        std::fprintf(stderr, "wgpu: sha256 pipeline failed\n");
        return false;
    }
...
extern "C" int kinet_sha256_wgpu_available(void) {
    return init_engine() ? 1 : 0;
}

extern "C" int sha256_batch_wgpu(
    ...
    size_t out_words = n * 8;             // 32 bytes per hash = 8 u32 words
    ...
    if (!wait_map(e.instance, e.device, buf_read, WGPUMapMode_Read, 0, out_bytes)) {
        std::fprintf(stderr, "wgpu: sha256 readback map failed\n");
        return -5;
    }
    const void* mapped = wgpuBufferGetConstMappedRange(buf_read, 0, out_bytes);
    std::memcpy(outputs_arena, mapped, n * 32);
...
#else // KINET_SHA256_HAS_WEBGPU not defined: stub mode

extern "C" int kinet_sha256_wgpu_available(void) { return 0; }
extern "C" int sha256_batch_wgpu(
