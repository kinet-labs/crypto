// CPU vs WGSL/WebGPU byte-equality test for batched SHA-256 (FIPS 180-4).
//
...
// WGSL kernel (sha256/gpu/wgsl/sha256.wgsl); outputs are compared
...
#include "../gpu/wgsl/sha256_driver_wgpu.h"
...
int main() {
    std::fprintf(stdout, "=== sha256 CPU vs WGSL byte-equality (FIPS 180-4) ===\n");

    if (!kinet_sha256_wgpu_available()) {
        std::fprintf(stdout,
    ...
    int rc = sha256_batch_wgpu(arena.data(), arena.size(),
