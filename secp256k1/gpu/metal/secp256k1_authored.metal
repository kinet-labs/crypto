...
// Field squaring: c = a^2 mod p (optimized Comba squaring)
// Exploits symmetry: 2*a[i]*a[j] for i != j
inline void fp_sqr(thread Fp256& c, Fp256 a) {
    // Full 512-bit product using Comba squaring
    uint64_t t[8] = {0};

    // Diagonal terms: a[i]^2
    for (int i = 0; i < 4; i++) {
        uint64_t lo = a.limbs[i] * a.limbs[i];
        uint64_t hi = mulhi(a.limbs[i], a.limbs[i]);

        uint64_t sum = t[2*i] + lo;
        uint64_t carry = (sum < t[2*i]) ? 1 : 0;
        t[2*i] = sum;

        sum = t[2*i + 1] + hi + carry;
        carry = (sum < t[2*i + 1] || (carry && sum == t[2*i + 1] + hi)) ? 1 : 0;
        t[2*i + 1] = sum;

        // Propagate carry
        for (int k = 2*i + 2; k < 8 && carry; k++) {
            sum = t[k] + carry;
            carry = (sum < t[k]) ? 1 : 0;
            t[k] = sum;
        }
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = i + 1; j < 4; j++) {
            uint64_t lo = a.limbs[i] * a.limbs[j];
            uint64_t hi = mulhi(a.limbs[i], a.limbs[j]);
            ...
            uint64_t sum = t[i+j] + lo;
            uint64_t carry = (sum < t[i+j]) ? 1 : 0;
            t[i+j] = sum;

            sum = t[i+j+1] + hi + carry;
            carry = (sum < t[i+j+1] || (carry && sum == t[i+j+1] + hi)) ? 1 : 0;
            t[i+j+1] = sum;
    ...
    uint64_t carry = 0;
    for (int i = 0; i < 4; i++) {
        uint64_t lo = t[i+4] * 0x1000003D1ULL;
        uint64_t hi = mulhi(t[i+4], 0x1000003D1ULL);
...
// Scalar addition: c = (a + b) mod n
inline void sc_add(thread Scalar256& c, Scalar256 a, Scalar256 b) {
    uint64_t carry = 0;
    for (int i = 0; i < 4; i++) {
    ...
    // Reduce mod n
    Scalar256 n = {{SECP256K1_N[0], SECP256K1_N[1], SECP256K1_N[2], SECP256K1_N[3]}};
    bool gte = true;
    for (int i = 3; i >= 0; i--) {
        if (c.limbs[i] > n.limbs[i]) break;
        if (c.limbs[i] < n.limbs[i]) { gte = false; break; }
    }
    if (gte || carry) {
        uint64_t borrow = 0;
        for (int i = 0; i < 4; i++) {
            uint64_t diff = c.limbs[i] - n.limbs[i] - borrow;
            borrow = (c.limbs[i] < n.limbs[i] + borrow) ? 1 : 0;
            c.limbs[i] = diff;
        }
    }
}

// Scalar subtraction: c = (a - b) mod n
inline void sc_sub(thread Scalar256& c, Scalar256 a, Scalar256 b) {
    bool gte = true;
    for (int i = 3; i >= 0; i--) {
        if (a.limbs[i] > b.limbs[i]) break;
        if (a.limbs[i] < b.limbs[i]) { gte = false; break; }
    }
    if (gte) {
        uint64_t borrow = 0;
        for (int i = 0; i < 4; i++) {
        ...
        // a < b, so compute a + n - b
        Scalar256 n = {{SECP256K1_N[0], SECP256K1_N[1], SECP256K1_N[2], SECP256K1_N[3]}};
        uint64_t carry = 0;
        for (int i = 0; i < 4; i++) {
            uint64_t sum = a.limbs[i] + n.limbs[i] + carry;
            carry = (sum < a.limbs[i] || (carry && sum == a.limbs[i])) ? 1 : 0;
            c.limbs[i] = sum;
        }
        uint64_t borrow = 0;
        for (int i = 0; i < 4; i++) {
            uint64_t diff = c.limbs[i] - b.limbs[i] - borrow;
            borrow = (c.limbs[i] < b.limbs[i] + borrow) ? 1 : 0;
            c.limbs[i] = diff;
        }
    }
}

// Scalar multiplication: c = a * b mod n
inline void sc_mul(thread Scalar256& c, Scalar256 a, Scalar256 b) {
    // Full 512-bit product
    uint64_t t[8] = {0};
    ...
    // For secp256k1, n is close to 2^256, so we use simple reduction
    Scalar256 n = {{SECP256K1_N[0], SECP256K1_N[1], SECP256K1_N[2], SECP256K1_N[3]}};

    // Reduce high 256 bits: multiply by 2^256 mod n and add to low
    // 2^256 mod n = 2^256 - n = 0x014551231950B75FC4402DA1732FC9BEBF
    constant uint64_t R_MOD_N[4] = {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j <= i; j++) {
            if (j < 4 && (i-j) < 4) {
                uint64_t plo = t[4+j] * R_MOD_N[i-j];
                uint64_t phi = mulhi(t[4+j], R_MOD_N[i-j]);
                uint64_t sum = lo + plo;
    ...
    // Handle remaining carry with reduction
    while (carry > 0) {
        uint64_t lo = carry * R_MOD_N[0];
        uint64_t hi = mulhi(carry, R_MOD_N[0]);

        uint64_t sum = t[0] + lo;
        uint64_t c1 = (sum < t[0]) ? 1 : 0;
        t[0] = sum;

        sum = t[1] + (carry * R_MOD_N[1]) + c1;
        c1 = (sum < t[1]) ? 1 : 0;
        t[1] = sum;
        ...
        carry = hi + mulhi(carry, R_MOD_N[1]) + c1;
    }

    // Final reduction if >= n
    c.limbs[0] = t[0];
    c.limbs[1] = t[1];
    c.limbs[2] = t[2];
    c.limbs[3] = t[3];
...
// Scalar inversion: c = a^(-1) mod n using Fermat's little theorem
// a^(-1) = a^(n-2) mod n
inline void sc_inv(thread Scalar256& c, Scalar256 a) {
    // n - 2 = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD036413F
    Scalar256 result = {{1, 0, 0, 0}};
    Scalar256 base = a;

    // Binary exponentiation with n-2
    constant uint64_t N_MINUS_2[4] = {
    ...
    for (int limb = 0; limb < 4; limb++) {
        uint64_t exp_limb = N_MINUS_2[limb];
        for (int bit = 0; bit < 64; bit++) {
            if ((exp_limb >> bit) & 1) {
                sc_mul(result, result, base);
            }
            Scalar256 tmp;
...
kernel void ecdsa_verify_batch(
    ...
    Scalar256 n = {{SECP256K1_N[0], SECP256K1_N[1], SECP256K1_N[2], SECP256K1_N[3]}};
    bool r_zero = (r.limbs[0] == 0 && r.limbs[1] == 0 && r.limbs[2] == 0 && r.limbs[3] == 0);
    bool s_zero = (s.limbs[0] == 0 && s.limbs[1] == 0 && s.limbs[2] == 0 && s.limbs[3] == 0);
    if (r_zero || s_zero) {
        results[index] = 0;  // Invalid signature
        return;
    }

    // Load message hash as scalar z (big-endian to little-endian)
    Scalar256 z;
    ...
    // Step 1: Compute s^(-1) mod n
    Scalar256 s_inv;
    sc_inv(s_inv, s);

    // Step 2: Compute u1 = z * s^(-1) mod n
    Scalar256 u1;
    sc_mul(u1, z, s_inv);
    ...
    // Step 4: Compute R = u1*G + u2*Q using double-and-add
    // For u1*G, use GTable if available, otherwise double-and-add with generator
    AffinePoint G;
    G.x = {{SECP256K1_GX[0], SECP256K1_GX[1], SECP256K1_GX[2], SECP256K1_GX[3]}};
    G.y = {{SECP256K1_GY[0], SECP256K1_GY[1], SECP256K1_GY[2], SECP256K1_GY[3]}};
    G.infinity = false;

    // Compute u1*G using GTable (16 table lookups + 15 additions)
    JacobianPoint acc_u1G;
    acc_u1G.x = {{0, 0, 0, 0}};
    acc_u1G.y = {{1, 0, 0, 0}};
    acc_u1G.z = {{0, 0, 0, 0}};
    bool started_u1 = false;

    for (uint32_t i = 0; i < 16; i++) {
        uint32_t bit_offset = i * 16;
        uint32_t limb_idx = bit_offset / 64;
        uint32_t limb_offset = bit_offset % 64;

        uint32_t chunk = (u1.limbs[limb_idx] >> limb_offset) & 0xFFFF;
        if (limb_offset > 48 && limb_idx < 3) {
            chunk |= (u1.limbs[limb_idx + 1] << (64 - limb_offset)) & 0xFFFF;
        }

        if (chunk != 0) {
            uint32_t table_idx = i * 65536 + (chunk - 1);
            AffinePoint p = gtable[table_idx];
            if (started_u1) {
                acc_u1G = point_add_mixed(acc_u1G, p);
            } else {
                acc_u1G = affine_to_jacobian(p);
                started_u1 = true;
            }
        }
    }

    // Compute u2*Q using double-and-add
    JacobianPoint acc_u2Q;
    acc_u2Q.x = {{0, 0, 0, 0}};
    acc_u2Q.y = {{1, 0, 0, 0}};
    acc_u2Q.z = {{0, 0, 0, 0}};
    bool started_u2 = false;

    for (int limb = 3; limb >= 0; limb--) {
        for (int bit = 63; bit >= 0; bit--) {
            if (started_u2) {
                acc_u2Q = point_double(acc_u2Q);
            }
            if ((u2.limbs[limb] >> bit) & 1) {
                if (started_u2) {
                    acc_u2Q = point_add_mixed(acc_u2Q, Q);
                } else {
                    acc_u2Q = affine_to_jacobian(Q);
                    started_u2 = true;
                }
            }
        }
    ...
    // Compute R = u1*G + u2*Q
    JacobianPoint R_jacobian;
    if (!started_u1 && !started_u2) {
        // Both are identity, invalid
        results[index] = 0;
        return;
    } else if (!started_u1) {
        R_jacobian = acc_u2Q;
        ...
        // Add the two points (need full projective addition)
        // Convert acc_u2Q to affine for mixed addition
        AffinePoint u2Q_affine = jacobian_to_affine(acc_u2Q);
        R_jacobian = point_add_mixed(acc_u1G, u2Q_affine);
    }

    // Check if R is point at infinity
    if (is_zero256(R_jacobian.z)) {
        results[index] = 0;
        return;
    }

    // Convert R to affine to get x-coordinate
    AffinePoint R = jacobian_to_affine(R_jacobian);

    // Step 5: Verify R.x mod n == r
    // R.x is already in field, need to reduce mod n
    Scalar256 rx;
    rx.limbs[0] = R.x.limbs[0];
    rx.limbs[1] = R.x.limbs[1];
    rx.limbs[2] = R.x.limbs[2];
    rx.limbs[3] = R.x.limbs[3];

    // Reduce rx mod n if >= n
    bool gte = true;
    for (int i = 3; i >= 0; i--) {
        if (rx.limbs[i] > n.limbs[i]) break;
        if (rx.limbs[i] < n.limbs[i]) { gte = false; break; }
    }
    if (gte) {
        uint64_t borrow = 0;
        for (int i = 0; i < 4; i++) {
            uint64_t diff = rx.limbs[i] - n.limbs[i] - borrow;
            borrow = (rx.limbs[i] < n.limbs[i] + borrow) ? 1 : 0;
            rx.limbs[i] = diff;
        }
    }

    // Compare rx with r
    bool equal = (rx.limbs[0] == r.limbs[0] && rx.limbs[1] == r.limbs[1] &&
                  rx.limbs[2] == r.limbs[2] && rx.limbs[3] == r.limbs[3]);

    results[index] = equal ? 1 : 0;
}
