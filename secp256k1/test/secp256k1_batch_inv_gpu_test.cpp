...
#if __APPLE__
    const char* metallib = std::getenv("KINET_CRYPTO_SECP256K1_BATCH_INV_METALLIB");
    if (!metallib) {
        std::fprintf(stdout, "(skip GPU equality: KINET_CRYPTO_SECP256K1_BATCH_INV_METALLIB unset)\n");
