...
auto srs = kinet::crypto::KZG::load_srs("mainnet_srs.dat");

// Or generate for testing (NOT for production!)
auto test_srs = kinet::crypto::KZG::generate_test_srs(4096);
```
...
auto poly = kinet::crypto::Polynomial(coeffs);

// Commit to polynomial
auto commitment = kinet::crypto::kzg_commit(srs, poly);
```
...
Scalar z = ...;
auto [proof, y] = kinet::crypto::kzg_open(srs, poly, z);

// Verify the proof
bool valid = kinet::crypto::kzg_verify(srs, commitment, z, y, proof);
```
...
auto commitments = kinet::crypto::kzg_commit_batch(srs, polys);

// Batch verify multiple openings (much faster)
std::vector<Opening> openings = {...};
bool all_valid = kinet::crypto::kzg_verify_batch(srs, openings);
```
...
auto commitment = kinet::crypto::kzg_commit_gpu(srs, poly);
```
...
auto evaluations = kinet::crypto::fft_gpu(poly);

// Interpolate from evaluations (inverse FFT)
auto poly2 = kinet::crypto::ifft_gpu(evaluations);
```
...
auto commitment = kinet::crypto::blob_to_kzg_commitment(srs, blob);
```
...
auto proof = kinet::crypto::compute_kzg_proof(srs, blob, z);

// Generate proof for blob (EIP-4844)
auto blob_proof = kinet::crypto::compute_blob_kzg_proof(srs, blob, commitment);
```
...
```cpp
// Verify blob proof
bool valid = kinet::crypto::verify_blob_kzg_proof(
    srs, blob, commitment, blob_proof
);

// Batch verify multiple blobs
bool all_valid = kinet::crypto::verify_blob_kzg_proof_batch(
...
auto tree = kinet::crypto::VerkleTree::build(srs, data);
...
// Verify multiproof
bool valid = kinet::crypto::verify_verkle_multiproof(
...
// Deserialize
auto commitment2 = kinet::crypto::KZGCommitment::from_bytes(bytes);
```
...
import "github.com/kinet-labs/crypto/go/kzg"
...
auto srs = kinet::crypto::KZG::load_srs("powers-of-tau-ceremony.dat");

// Testing only: Generate locally (INSECURE!)
auto test_srs = kinet::crypto::KZG::generate_test_srs(n);
```
...
// Manual check
bool valid = kinet::crypto::is_in_g1(point);
```
