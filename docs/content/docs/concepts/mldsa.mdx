...
kinet-crypto implements all three NIST security levels:
...
auto [sk, pk] = kinet::crypto::mldsa_keygen<kinet::crypto::MLDSA65>();

// Or specify security level explicitly
auto [sk44, pk44] = kinet::crypto::mldsa_keygen<kinet::crypto::MLDSA44>();
auto [sk87, pk87] = kinet::crypto::mldsa_keygen<kinet::crypto::MLDSA87>();
```
...
auto sig = kinet::crypto::mldsa_sign(sk, message);

// Verify signature
bool valid = kinet::crypto::mldsa_verify(pk, message, sig);
```
...
auto sig = kinet::crypto::mldsa_sign_deterministic(sk, message);
```
...
auto t = kinet::crypto::mldsa_matrix_mul(A, s);
```
...
For applications transitioning from classical to post-quantum cryptography, kinet-crypto supports hybrid schemes:

```cpp
#include "hybrid.hpp"

// Generate hybrid key pair (BLS + ML-DSA)
auto [sk, pk] = kinet::crypto::hybrid_keygen();

// Hybrid signature contains both BLS and ML-DSA signatures
auto sig = kinet::crypto::hybrid_sign(sk, message);

// Verification requires both signatures to be valid
bool valid = kinet::crypto::hybrid_verify(pk, message, sig);
```
...
bool in_range = kinet::crypto::ct_less_than(sample, bound);
```
...
auto [sk, pk] = kinet::crypto::mldsa_keygen<MLDSA65>();

// Or provide custom RNG
auto [sk, pk] = kinet::crypto::mldsa_keygen<MLDSA65>(custom_rng);
```
...
import "github.com/kinet-labs/crypto/go/mldsa"
