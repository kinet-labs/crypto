...
kinet-crypto provides GPU-accelerated implementations of modern cryptographic hash functions, with BLAKE3 as the primary hash algorithm.
...
auto hash = kinet::crypto::blake3(message);

// Hash with custom output length
auto hash_512 = kinet::crypto::blake3(message, 64); // 64-byte output
```
...
```cpp
// Initialize hasher
auto hasher = kinet::crypto::Blake3Hasher();
...
auto mac = kinet::crypto::blake3_keyed(key, message);

// Verify MAC
bool valid = kinet::crypto::ct_equal(mac, expected_mac);
```
...
std::string context = "kinet-crypto v1 encryption key";
auto derived = kinet::crypto::blake3_derive_key(context, input_key, 32);
```
...
For elliptic curve operations, kinet-crypto provides hash-to-curve functions per RFC 9380:
...
auto point = kinet::crypto::hash_to_g1(message, dst);
```
...
auto point = kinet::crypto::hash_to_g2(message, dst);
```
...
auto tree = kinet::crypto::MerkleTree::build(leaves);
...
// Verify proof
bool valid = kinet::crypto::verify_merkle_proof(root, leaf, proof);
```
...
auto tree = kinet::crypto::MerkleTree::build_gpu(leaves);
```
...
auto hash = kinet::crypto::sha256(message);
```
...
auto hash = kinet::crypto::keccak256(message);
```
...
auto hash = kinet::crypto::poseidon(field_elements);
```
...
import "github.com/kinet-labs/crypto/go/hash"
...
bool valid = kinet::crypto::ct_equal(hash, expected);
