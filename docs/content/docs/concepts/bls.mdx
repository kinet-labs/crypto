...
Signature agg_sig = kinet::crypto::bls_aggregate(sigs);

// Verify aggregated signature against all public keys
bool valid = kinet::crypto::bls_aggregate_verify(
...
auto [sk, pk] = kinet::crypto::bls_keygen();

// Derive public key from secret key
PublicKey pk2 = kinet::crypto::bls_sk_to_pk(sk);
```
...
Signature sig = kinet::crypto::bls_sign(sk, message);

// Verify signature
bool valid = kinet::crypto::bls_verify(pk, message, sig);
```
...
Signature agg = kinet::crypto::bls_aggregate(signatures);

// Verify aggregate (much faster than verifying individually)
bool valid = kinet::crypto::bls_aggregate_verify(pubkeys, messages, agg);
```
...
// e(sig, G2) == e(H(msg), pk)
bool valid = kinet::crypto::bls_pairing_check(sig, g2_gen, hash, pk);
```
...
ProofOfPossession pop = kinet::crypto::bls_pop_prove(sk);

// Verify proof before accepting public key
bool valid_pop = kinet::crypto::bls_pop_verify(pk, pop);
```
...
G2Point h = kinet::crypto::hash_to_g2(message, dst);
```
...
auto [sk, pk] = kinet::crypto::bn254_keygen();
auto sig = kinet::crypto::bn254_sign(sk, message);
```
...
import "github.com/kinet-labs/crypto/go/bls"
