---
title: API Reference
description: Complete API documentation for kinet-crypto
---

# API Reference

This section provides comprehensive documentation for all public APIs in kinet-crypto.
...
```go
import (
    "github.com/kinet-labs/crypto/go/bls"
    "github.com/kinet-labs/crypto/go/mldsa"
    "github.com/kinet-labs/crypto/go/hash"
    "github.com/kinet-labs/crypto/go/kzg"
)
```
...
```cpp
try {
    auto sig = kinet::crypto::bls_sign(sk, message);
} catch (const kinet::crypto::InvalidKeyError& e) {
    // Handle invalid key
} catch (const kinet::crypto::CryptoError& e) {
...
All kinet-crypto operations are thread-safe. Internal state uses appropriate synchronization:

```cpp
// Safe to call from multiple threads
auto sig1 = kinet::crypto::bls_sign(sk, msg1); // Thread 1
auto sig2 = kinet::crypto::bls_sign(sk, msg2); // Thread 2
```
