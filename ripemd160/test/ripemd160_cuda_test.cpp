// CPU vs CUDA byte-equality test for batched RIPEMD-160 (Dobbertin et al. 1996).
//
...
// the CUDA kernel (ripemd160/gpu/cuda/ripemd160.cu); outputs are compared
...
#include "../gpu/cuda/ripemd160_driver.h"
...
int main() {
    std::fprintf(stdout, "=== ripemd160 CPU vs CUDA byte-equality (Dobbertin 1996) ===\n");

    if (!kinet_ripemd160_cuda_available()) {
        std::fprintf(stdout,
    ...
    int rc = ripemd160_batch_cuda(arena.data(), arena.size(),
