// Metal driver for batched BLAKE3 (BLAKE3 spec). macOS / iOS only.
// Loads blake3_batch.metallib, dispatches `blake3_jobs` with one thread per
// input. Byte-equal to blake3/cpp/blake3.cpp::hash().
...
extern "C" int blake3_batch_metal(
    ...
    @autoreleasepool {
        ...
        id<MTLFunction> fn = [lib newFunctionWithName:@"blake3_jobs"];
        if (!fn) return -4;
        ...
        std::vector<Blake3JobGPU> jobs(n);
        ...
        id<MTLBuffer> jobs_buf = [device newBufferWithBytes:jobs.data()
                                                    length:jobs.size() * sizeof(Blake3JobGPU)
