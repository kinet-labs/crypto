// CPU-oracle vs CUDA byte-equality for the tree-reduce Pedersen vector
// commitment at the fixed Verkle width N = 256.
//
// Skips silently when no CUDA device is present (the host driver returns
// -1 from kinet_pedersen_tree_cuda_available()).

#include "../gpu/cuda/pedersen_tree_driver.h"
...
int main() {
    std::fprintf(stdout, "=== pedersen tree-reduce CPU-oracle vs CUDA byte-equality ===\n");

    if (!kinet_pedersen_tree_cuda_available()) {
        std::fprintf(stdout, "(skip: no CUDA device)\n");
    ...
    for (int round = 0; round < target_rounds; ++round) {
        ...
        if (rc != 0) {
            std::fprintf(stderr, "FAIL CUDA dispatch round=%d rc=%d\n", round, rc);
            return 1;
        }
    ...
    if (target_rounds > 0) {
        ...
        std::fprintf(stdout, "timing  CUDA tree dispatch (M=%u): %8.3f us / batch (%d rounds)\n",
