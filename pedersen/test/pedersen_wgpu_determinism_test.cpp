// CPU-oracle vs WebGPU/WGSL byte-equality test for batched Pedersen vector
// commitments over BN254 G1.  Uses the same KAT fixture as the Metal and CUDA
// tests.

#include "../gpu/wgsl/pedersen_driver_wgpu.h"
...
int main() {
    std::fprintf(stdout, "=== pedersen vector-commit CPU-oracle vs WebGPU/WGSL byte-equality ===\n");

    if (!kinet_pedersen_wgpu_available()) {
        std::fprintf(stdout, "(skip: WebGPU runtime not available)\n");
    ...
    for (uint32_t round = 0; round < ROUNDS; ++round) {
        ...
        int rc = pedersen_batch_wgpu(
            gens_be, scalars_be, blindings_be, M, N, gpu_out.data());
        if (rc != 0) {
            std::fprintf(stderr, "FAIL WebGPU dispatch round=%u rc=%d\n", round, rc);
            return 1;
        }
    ...
    std::fprintf(stdout, "PASS WebGPU dispatch all rounds rc=0\n");
    ...
    std::fprintf(stdout, "PASS 100/100 rounds byte-equal across CPU oracle and WebGPU/WGSL kernel\n");
