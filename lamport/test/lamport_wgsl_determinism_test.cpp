// CPU vs WGSL-kernel byte-equality test for Lamport-SHA256 OTS. The WGSL
// kernel is in lamport/gpu/wgsl/lamport.wgsl; its arithmetic is replayed on
// the host by lamport/gpu/wgsl/lamport_wgsl_oracle.cpp so the test runs on
...
// every preimage slot (sk + sig) through the WGSL oracle and assert
// byte-equality with the canonical CPU body.
...
uint64_t lcg_state = 0xABCDEF0123456789ULL;
uint8_t lcg_byte() {
...
int main() {
    std::fprintf(stdout, "=== lamport CPU vs WGSL byte-equality (LP-2506) ===\n");
    ...
    lamport_hash_jobs_wgsl_oracle(slots.data(), gpu_dig.data(),
                                  (uint32_t)total_slots);
    std::fprintf(stdout, "PASS WGSL oracle dispatch (%zu slots)\n", total_slots);
