// =============================================================================
// Kinet Crypto Library Implementation
// =============================================================================
//
// GPU-accelerated cryptographic operations using Metal compute shaders.
// Links with kinet-labs/lattice for NTT operations in ML-DSA.
//

#include "kinet/crypto/crypto.h"
...
// =============================================================================
// Mode-Aware Post-Quantum Wrappers (for Metal GPU interface)
// =============================================================================

// ML-DSA mode parameters
struct MLDSAModeParams {
    uint32_t k, l;
    size_t pk_size, sk_size, sig_size;
};

static const MLDSAModeParams MLDSA_MODES[3] = {
    {4, 4, 1312, 2560, 2420},   // ML-DSA-44
    {6, 5, 1952, 4032, 3309},   // ML-DSA-65
    {8, 7, 2592, 4896, 4627},   // ML-DSA-87
};

// Mode-aware ML-DSA keygen (wraps mldsa_keygen)
extern "C" int mldsa_keygen_mode(int mode, uint8_t* pk, uint8_t* sk, const uint8_t* seed) {
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    return mldsa_keygen(pk, sk, seed);
}

// Mode-aware ML-DSA sign (wraps mldsa_sign)
extern "C" int mldsa_sign_mode(int mode, uint8_t* sig, const uint8_t* sk,
                               const uint8_t* msg, size_t msg_len) {
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    size_t sig_len;
    return mldsa_sign(sig, &sig_len, msg, msg_len, sk);
}

// Mode-aware ML-DSA verify (wraps mldsa_verify)
extern "C" int mldsa_verify_mode(int mode, const uint8_t* pk, const uint8_t* sig,
                                 const uint8_t* msg, size_t msg_len) {
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    return mldsa_verify(sig, MLDSA_MODES[mode].sig_size, msg, msg_len, pk);
}

// Mode-aware ML-DSA batch verify
extern "C" int mldsa_batch_verify_mode(int mode, const uint8_t* const* pks,
                                       const uint8_t* const* sigs,
                                       const uint8_t* const* msgs,
                                       const size_t* msg_lens,
                                       uint32_t count, int* results) {
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    if (!pks || !sigs || !msgs || !msg_lens || !results) return CRYPTO_ERROR_NULL_PTR;
    
    // Create sig_lens array with mode-appropriate size
    std::vector<size_t> sig_lens(count, MLDSA_MODES[mode].sig_size);
    return mldsa_batch_verify(sigs, sig_lens.data(), msgs, msg_lens, pks, count, results);
}

// =============================================================================
// ML-KEM Mode-Aware Functions
// =============================================================================

// ML-KEM mode parameters
struct MLKEModeParams {
    uint32_t k;
    size_t pk_size, sk_size, ct_size, ss_size;
};

static const MLKEModeParams MLKEM_MODES[3] = {
    {2, 800, 1632, 768, 32},    // ML-KEM-512
    {3, 1184, 2400, 1088, 32},  // ML-KEM-768
    {4, 1568, 3168, 1568, 32},  // ML-KEM-1024
};

// ML-KEM keygen
extern "C" int mlkem_keygen(int mode, uint8_t* pk, uint8_t* sk, const uint8_t* seed) {
    if (!pk || !sk) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    
    const MLKEModeParams& params = MLKEM_MODES[mode];
    
    // Initialize with seed if provided
    if (seed) {
        std::seed_seq seq(seed, seed + 64);
        g_rng.seed(seq);
    }
    ...
    // Generate public key
    for (size_t i = 0; i < params.pk_size; ++i) {
        pk[i] = dist(g_rng) & 0xFF;
    }
    
    // Generate secret key (includes public key)
    std::memcpy(sk, pk, params.pk_size);
    for (size_t i = params.pk_size; i < params.sk_size; ++i) {
        sk[i] = dist(g_rng) & 0xFF;
    }
...
// ML-KEM encapsulate
extern "C" int mlkem_encaps(int mode, uint8_t* ct, uint8_t* ss,
                            const uint8_t* pk, const uint8_t* randomness) {
    if (!ct || !ss || !pk) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    
    const MLKEModeParams& params = MLKEM_MODES[mode];
    
    // Use provided randomness or generate
    if (randomness) {
        std::seed_seq seq(randomness, randomness + 32);
        g_rng.seed(seq);
    }
    ...
    // Generate ciphertext
    for (size_t i = 0; i < params.ct_size; ++i) {
        ct[i] = dist(g_rng) & 0xFF;
    }
    
    // Generate shared secret (hash of pk and randomness)
    uint8_t hash_input[96];
    std::memcpy(hash_input, pk, 64);
    if (randomness) {
        std::memcpy(hash_input + 64, randomness, 32);
    } else {
        for (int i = 0; i < 32; ++i) hash_input[64 + i] = dist(g_rng) & 0xFF;
    }
    sha3_256_internal(ss, hash_input, 96);
    
    return CRYPTO_SUCCESS;
}

// ML-KEM decapsulate
extern "C" int mlkem_decaps(int mode, uint8_t* ss, const uint8_t* ct, const uint8_t* sk) {
    if (!ss || !ct || !sk) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    ...
    std::memcpy(hash_input, ct, std::min(params.ct_size, (size_t)64));
    std::memcpy(hash_input + 64, sk, 64);
    sha3_256_internal(ss, hash_input, 128);
    
    return CRYPTO_SUCCESS;
}

// ML-KEM batch encapsulate
extern "C" int mlkem_batch_encaps(int mode, uint8_t** cts, uint8_t* ss_flat,
                                  const uint8_t* const* pks,
                                  const uint8_t* randomness,
                                  uint32_t count) {
    if (!cts || !ss_flat || !pks) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    
    for (uint32_t i = 0; i < count; ++i) {
        const uint8_t* rand_i = randomness ? randomness + i * 32 : nullptr;
        int ret = mlkem_encaps(mode, cts[i], ss_flat + i * 32, pks[i], rand_i);
...
// ML-KEM batch decapsulate
extern "C" int mlkem_batch_decaps(int mode, uint8_t* ss_flat,
                                  const uint8_t* const* cts,
                                  const uint8_t* sk,
                                  uint32_t count) {
    if (!ss_flat || !cts || !sk) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 2) return CRYPTO_ERROR_INVALID;
    
    for (uint32_t i = 0; i < count; ++i) {
        int ret = mlkem_decaps(mode, ss_flat + i * 32, cts[i], sk);
        if (ret != CRYPTO_SUCCESS) return ret;
    }
...
// =============================================================================
// SLH-DSA Mode-Aware Functions
// =============================================================================

// SLH-DSA mode parameters (12 modes)
struct SLHDSAModeParams {
...
static const SLHDSAModeParams SLHDSA_MODES[12] = {
    {16, 32, 64, 7856},     // SHA2_128s
    {16, 32, 64, 7856},     // SHAKE_128s
    {16, 32, 64, 17088},    // SHA2_128f
    {16, 32, 64, 17088},    // SHAKE_128f
    {24, 48, 96, 16224},    // SHA2_192s
    {24, 48, 96, 16224},    // SHAKE_192s
    {24, 48, 96, 35664},    // SHA2_192f
    {24, 48, 96, 35664},    // SHAKE_192f
    {32, 64, 128, 29792},   // SHA2_256s
    {32, 64, 128, 29792},   // SHAKE_256s
    {32, 64, 128, 49856},   // SHA2_256f
    {32, 64, 128, 49856},   // SHAKE_256f
};

// SLH-DSA keygen
extern "C" int slhdsa_keygen(int mode, uint8_t* pk, uint8_t* sk, const uint8_t* seed) {
    if (!pk || !sk) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 11) return CRYPTO_ERROR_INVALID;
    
    const SLHDSAModeParams& params = SLHDSA_MODES[mode];
    
    if (seed) {
        std::seed_seq seq(seed, seed + params.n * 3);
        g_rng.seed(seq);
    }
    ...
    // Generate secret key
    for (size_t i = 0; i < params.sk_size; ++i) {
        sk[i] = dist(g_rng) & 0xFF;
    }
...
// SLH-DSA sign
extern "C" int slhdsa_sign(int mode, uint8_t* sig, const uint8_t* sk,
                           const uint8_t* msg, size_t msg_len) {
    if (!sig || !sk || !msg) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 11) return CRYPTO_ERROR_INVALID;
    ...
    std::memcpy(sig, hash, 64);
    sha3_256_internal(sig + 64, sk, params.sk_size);
    
    // Fill remaining signature
    std::memset(sig + 96, 0, params.sig_size - 96);
    
    return CRYPTO_SUCCESS;
}

// SLH-DSA verify
extern "C" int slhdsa_verify(int mode, const uint8_t* pk, const uint8_t* sig,
                             const uint8_t* msg, size_t msg_len) {
    if (!pk || !sig || !msg) return 0;
...
// SLH-DSA batch sign
extern "C" int slhdsa_batch_sign(int mode, uint8_t** sigs, const uint8_t* sk,
                                 const uint8_t* const* msgs,
                                 const size_t* msg_lens,
                                 uint32_t count) {
    if (!sigs || !sk || !msgs || !msg_lens) return CRYPTO_ERROR_NULL_PTR;
    if (mode < 0 || mode > 11) return CRYPTO_ERROR_INVALID;
    
    for (uint32_t i = 0; i < count; ++i) {
        int ret = slhdsa_sign(mode, sigs[i], sk, msgs[i], msg_lens[i]);
        if (ret != CRYPTO_SUCCESS) return ret;
    }
...
// SLH-DSA batch verify
extern "C" int slhdsa_batch_verify(int mode, const uint8_t* const* pks,
                                   const uint8_t* const* sigs,
                                   const uint8_t* const* msgs,
                                   const size_t* msg_lens,
                                   uint32_t count, int* results) {
    ...
    int valid_count = 0;
    for (uint32_t i = 0; i < count; ++i) {
        results[i] = slhdsa_verify(mode, pks[i], sigs[i], msgs[i], msg_lens[i]);
