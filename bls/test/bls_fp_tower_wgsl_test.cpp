...
int main(int /*argc*/, char** /*argv*/) {
    ...
    auto bin = [&](const char* op, const char* metric, size_t elem,
        ...
        if (kinet_bls_wgpu_run_binary(op, A, B, buf.data(), elem, kNVectors) != 0) {
    ...
    auto un = [&](const char* op, const char* metric, size_t elem,
        ...
        buf.assign(kNVectors * elem, 0);
        if (kinet_bls_wgpu_run_unary(op, A, buf.data(), elem, kNVectors) != 0) {
            std::fprintf(stderr, "FATAL: dispatch %s failed\n", op);
            std::exit(2);
        }
        results.push_back(cmp(metric, elem, buf.data(), O, kNVectors));
    ...
    // k_fp6_inv stack-budget exceeded on AGXMetalG13X — see Stage 4 honest gap.
    ...
    // k_fp12_mul/sqr/inv/conj/cyclo_sqr stack-budget exceeded on AGXMetalG13X.
    ...
    if (total_pass > 0) {
        ...
        kinet_bls_wgpu_run_binary("k_fp2_mul", kFp2A_bytes, kFp2B_bytes,
