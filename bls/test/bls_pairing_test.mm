...
// Single command-buffer / encoder scope. While a Submit is alive,
// dispatch_pso() encodes into the shared encoder instead of acquiring
// per-dispatch cb+enc and round-tripping. Apple guarantees in-order
// execution within one encoder, so byte-equality is preserved.
//
// The Stage 3 dispatch profile is ~280 kernels per pairing. With per-
// dispatch commit/wait, host-GPU latency dominates (~28 ms/pairing).
// With one cb+enc spanning the full computation, we pay one round trip.
id<MTLCommandBuffer>          g_cb  = nil;
...
// Open one cb+enc that subsequent dispatch_pso() calls encode into.
// Caller must invoke submit_end() to commit + wait.
void submit_begin()
{
                  ...
                  size_t count)
{
    ...
    if (g_enc != nil) {
        // Encode into the shared command buffer / encoder.
        [g_enc setComputePipelineState:pso];
        for (NSUInteger i = 0; i < bufs.size(); i++) {
            [g_enc setBuffer:bufs[i] offset:0 atIndex:i];
        }
        [g_enc dispatchThreads:MTLSizeMake(count, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(tg, 1, 1)];
        return;
    }
    // Fallback: standalone dispatch (used outside submit_begin/end scope,
    // e.g. for one-off helpers). Behaviour is identical to pre-change.
...
// =============================================================================
// Single-pairing latency micro-benchmark.
//
// Two modes, same code path, identical kernels and dispatch order:
//   * legacy   — each kernel commits and waits (g_enc == nil).
//   * single   — one cb+enc spans the whole pairing (g_enc set via submit_*).
//
// 1 warm-up + 10 measured runs, median printed in microseconds.
// =============================================================================
uint64_t bench_one(PSOs& p, bool single_cb)
{
    PairCtx c = make_ctx(p, 1);
    id<MTLBuffer> bIn  = make_buf(kPair_GenIn,  kPairP2Aff + kPairP1Aff);
    id<MTLBuffer> bOut = make_zero(kFp12Bytes);
...
uint64_t median_of_runs(PSOs& p, bool single_cb, int runs)
{
    (void)bench_one(p, single_cb);  // warm-up (PSO/kernel JIT, allocator)
    std::vector<uint64_t> samples;
    samples.reserve(runs);
    for (int i = 0; i < runs; i++) samples.push_back(bench_one(p, single_cb));
    std::sort(samples.begin(), samples.end());
    return samples[samples.size() / 2];
}

void bench_single_pairing(PSOs& p)
{
    constexpr int kRuns = 10;
    uint64_t legacy = median_of_runs(p, false, kRuns);
    uint64_t single = median_of_runs(p, true,  kRuns);

    printf("\n--- single-pairing latency (median of %d runs) ---\n", kRuns);
    printf("  per-dispatch-commit (pre-change baseline) : %llu us\n",
           (unsigned long long)legacy);
    printf("  single cb+enc       (post-change)         : %llu us\n",
           (unsigned long long)single);
    if (single > 0) {
        double speedup = static_cast<double>(legacy) / static_cast<double>(single);
        printf("  speedup                                   : %.2fx\n", speedup);
    }
    printf("  blst host reference                       : ~510 us\n");
    printf("---------------------------------------------------\n\n");
}
