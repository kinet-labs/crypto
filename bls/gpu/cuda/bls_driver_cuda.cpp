...
//
// Build modes:
//   1. With CUDA toolkit (BLS_HAVE_CUDA defined):
...
//
//   2. Without CUDA (BLS_HAVE_CUDA not defined):
...
// BLS_HAVE_CUDA=ON, which exercises the full byte-equality path against
...
int bls_cuda_available(void) { return bls_cuda::device_available(); }

int bls_cuda_fp2_mul(const void* a, const void* b, void* out, unsigned n) {
    return bls_cuda::dispatch3(k_fp2_mul, a, b, out, 96 * n, n);
}
int bls_cuda_fp12_mul(const void* a, const void* b, void* out, unsigned n) {
    return bls_cuda::dispatch3(k_fp12_mul, a, b, out, 576 * n, n);
}
...
int bls_cuda_pairing(const void* in_buf, void* out_buf, unsigned N) {
    if (!bls_cuda::device_available()) return -1;
    ...
    unsigned tg = 16;
    unsigned grid = bls_cuda::compute_grid(N, tg);
...
#else // BLS_HAVE_CUDA not defined: stub mode

extern "C" {
int bls_cuda_available(void) { return 0; }
int bls_cuda_fp2_mul(const void*, const void*, void*, unsigned) { return -1; }
int bls_cuda_fp12_mul(const void*, const void*, void*, unsigned) { return -1; }
int bls_cuda_pairing(const void*, void*, unsigned) { return -1; }
}
