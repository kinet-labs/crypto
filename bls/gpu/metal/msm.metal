...
// Uses add-2007-bl formula from hyperelliptic.org
inline G1Projective254 g1_add_projective_254(G1Projective254 p, G1Projective254 q) {
    // Check for identity points
    bool p_inf = (p.z.limbs[0] == 0 && p.z.limbs[1] == 0 &&
                  p.z.limbs[2] == 0 && p.z.limbs[3] == 0);
    bool q_inf = (q.z.limbs[0] == 0 && q.z.limbs[1] == 0 &&
                  q.z.limbs[2] == 0 && q.z.limbs[3] == 0);

    if (p_inf) return q;
    if (q_inf) return p;

    // U1 = X1*Z2^2, U2 = X2*Z1^2
    Fp254 z1z1, z2z2, u1, u2;
    fp254_mul(z1z1, p.z, p.z);
    fp254_mul(z2z2, q.z, q.z);
    fp254_mul(u1, p.x, z2z2);
    fp254_mul(u2, q.x, z1z1);

    // S1 = Y1*Z2^3, S2 = Y2*Z1^3
    Fp254 z1z1z1, z2z2z2, s1, s2;
    fp254_mul(z1z1z1, z1z1, p.z);
    fp254_mul(z2z2z2, z2z2, q.z);
    fp254_mul(s1, p.y, z2z2z2);
    fp254_mul(s2, q.y, z1z1z1);
    ...
    // Check if P = Q (need doubling) or P = -Q (infinity)
    ...
    G1Projective254 result;
    Fp254 r2r2, two_v;
    fp254_mul(r2r2, r2, r2);
    fp254_sub(result.x, r2r2, j);
    fp254_add(two_v, v, v);
    fp254_sub(result.x, result.x, two_v);

    // Y3 = r2*(V - X3) - 2*S1*J
    Fp254 v_minus_x3, s1j, two_s1j;
    fp254_sub(v_minus_x3, v, result.x);
    fp254_mul(result.y, r2, v_minus_x3);
    fp254_mul(s1j, s1, j);
    fp254_add(two_s1j, s1j, s1j);
    fp254_sub(result.y, result.y, two_s1j);

    // Z3 = ((Z1+Z2)^2 - Z1Z1 - Z2Z2)*H
    Fp254 z1_plus_z2, z1_plus_z2_sq, tmp;
    fp254_add(z1_plus_z2, p.z, q.z);
    fp254_mul(z1_plus_z2_sq, z1_plus_z2, z1_plus_z2);
    fp254_sub(tmp, z1_plus_z2_sq, z1z1);
    fp254_sub(tmp, tmp, z2z2);
    fp254_mul(result.z, tmp, h);
...
kernel void msm_bucket_reduce(
    ...
    for (int32_t i = MSM_BUCKETS_PER_WINDOW - 2; i >= 0; i--) {
        ...
        // Add bucket to running (full projective addition)
        running = g1_add_projective_254(running, bucket);
        sum = g1_add_projective_254(sum, running);
    }
...
kernel void msm_window_combine(
    ...
    for (int32_t i = num_windows - 2; i >= 0; i--) {
        ...
        // Add window sum
        acc = g1_add_projective_254(acc, window_sums[i]);
    }
...
kernel void pedersen_commit_bn254(
    ...
    // Add vG + rH
    commitments[index] = g1_add_projective_254(vG, rH);
}
...
kernel void vector_commit(
    ...
    for (uint32_t i = start; i < end; i++) {
        ...
        if (point_started) {
            if (started) {
                acc = g1_add_projective_254(acc, vG);
            } else {
                acc = vG;
                started = true;
            }
