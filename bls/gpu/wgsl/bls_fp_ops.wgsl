...
// Private scratch shared across the fp_* leaves so we don't materialise a
// fresh 96-byte fp_mul stack frame on every call from the Fp12 call tree.
// Workgroup size is 1 in every kernel that touches the upper tower, so each
// invocation owns these slots without contention.
var<private> fp_scratch_t: array<u32, 24>;
var<private> fp_scratch_a: array<u32, 26>;
var<private> fp_scratch_inv_exp: array<u32, 12>;
var<private> fp_scratch_inv_result: array<u32, 12>;
var<private> fp_scratch_inv_base: array<u32, 12>;

fn u384_mul768_priv(a: array<u32, 12>, b: array<u32, 12>,
                    t: ptr<private, array<u32, 24>>) {
    for (var i = 0u; i < 24u; i = i + 1u) { (*t)[i] = 0u; }
    for (var i = 0u; i < 12u; i = i + 1u) {
        var carry: u32 = 0u;
        for (var j = 0u; j < 12u; j = j + 1u) {
...
fn mont_reduce_384_priv(t: ptr<private, array<u32, 24>>,
                        r: ptr<function, array<u32, 12>>) {
    for (var i = 0u; i < 24u; i = i + 1u) { fp_scratch_a[i] = (*t)[i]; }
    fp_scratch_a[24] = 0u;
    fp_scratch_a[25] = 0u;

    for (var i = 0u; i < 12u; i = i + 1u) {
        let u = fp_scratch_a[i] * BLS_P_INV_LO;
        var carry: u32 = 0u;
        for (var j = 0u; j < 12u; j = j + 1u) {
            let prod = mul32_64(u, BLS_P[j]);
            var s = prod.x + carry;
            var c1: u32 = 0u; if (s < prod.x) { c1 = 1u; }
            let s2 = s + fp_scratch_a[i + j];
            var c2: u32 = 0u; if (s2 < s) { c2 = 1u; }
            fp_scratch_a[i + j] = s2;
            carry = prod.y + c1 + c2;
        }
        var k = i + 12u;
        while (carry != 0u && k < 26u) {
            let sum = fp_scratch_a[k] + carry;
            var c: u32 = 0u; if (sum < fp_scratch_a[k]) { c = 1u; }
            fp_scratch_a[k] = sum;
            carry = c;
            k = k + 1u;
        }
    ...
    var rr: array<u32, 12>;
    for (var i = 0u; i < 12u; i = i + 1u) { rr[i] = fp_scratch_a[12u + i]; }
    let cmp = u384_cmp(rr, BLS_P);
    if (fp_scratch_a[24] != 0u || cmp >= 0) {
        var sub: array<u32, 12>;
        _ = u384_sub(rr, BLS_P, &sub);
        rr = sub;
    }
...
fn fp_mul(a: array<u32, 12>, b: array<u32, 12>) -> array<u32, 12> {
    u384_mul768_priv(a, b, &fp_scratch_t);
    var r: array<u32, 12>;
    mont_reduce_384_priv(&fp_scratch_t, &r);
    return r;
}
...
fn fp_inv(a: array<u32, 12>) -> array<u32, 12> {
    for (var i = 0u; i < 12u; i = i + 1u) { fp_scratch_inv_exp[i] = BLS_P[i]; }
    // exp -= 2 on the lowest limb (low limb is well above 2)
    fp_scratch_inv_exp[0] = fp_scratch_inv_exp[0] - 2u;

    for (var i = 0u; i < 12u; i = i + 1u) { fp_scratch_inv_result[i] = BLS_R[i]; }
    var started: bool = false;
    for (var i = 11i; i >= 0; i = i - 1) {
        let ui = u32(i);
        for (var bit = 31i; bit >= 0; bit = bit - 1) {
            if (started) {
                var rcopy: array<u32, 12>;
                for (var k = 0u; k < 12u; k = k + 1u) { rcopy[k] = fp_scratch_inv_result[k]; }
                let s = fp_sqr(rcopy);
                for (var k = 0u; k < 12u; k = k + 1u) { fp_scratch_inv_result[k] = s[k]; }
            }
            let mask = 1u << u32(bit);
            if ((fp_scratch_inv_exp[ui] & mask) != 0u) {
                if (started) {
                    var rcopy: array<u32, 12>;
                    for (var k = 0u; k < 12u; k = k + 1u) { rcopy[k] = fp_scratch_inv_result[k]; }
                    let m = fp_mul(rcopy, a);
                    for (var k = 0u; k < 12u; k = k + 1u) { fp_scratch_inv_result[k] = m[k]; }
                } else {
                    for (var k = 0u; k < 12u; k = k + 1u) { fp_scratch_inv_result[k] = a[k]; }
                    started = true;
                }
            }
        }
    }
    var result: array<u32, 12>;
    for (var i = 0u; i < 12u; i = i + 1u) { result[i] = fp_scratch_inv_result[i]; }
    return result;
}
