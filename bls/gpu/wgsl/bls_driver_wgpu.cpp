...
//
// Build:
//   * KINET_BLS_HAS_WEBGPU=1                — Dawn or wgpu-native runtime found
//   * KINET_BLS_HAS_WGPU_NATIVE=1           — wgpu-native specifically (gives wgpuDevicePoll)
//
// On Apple host with Homebrew wgpu-native, both flags are set by CMake.

#include "bls_driver_wgpu.h"

#if defined(KINET_BLS_HAS_WEBGPU)

#include <webgpu.h>
#if defined(KINET_BLS_HAS_WGPU_NATIVE)
#  include <wgpu.h>
#endif
...
#else // KINET_BLS_HAS_WEBGPU not defined: stub mode

extern "C" {
int kinet_bls_wgpu_available(void) { return 0; }
int kinet_bls_wgpu_run_binary(const char*, const void*, const void*, void*, unsigned, unsigned) { return -1; }
int kinet_bls_wgpu_run_unary(const char*, const void*, void*, unsigned, unsigned) { return -1; }
}
