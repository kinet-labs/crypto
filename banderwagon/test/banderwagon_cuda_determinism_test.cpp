// CPU vs CUDA byte-equality determinism test for Banderwagon group ops.
//
// Three batteries identical to the Metal test:
//   1. 100 random Element pairs   : CPU::add(P, Q)        == CUDA add
//   2. 100 random (Element, Fr)   : CPU::scalar_mul(P, s) == CUDA smul
//   3. 5 batch MSMs (sizes 64, 256, 1024, varying batch widths)
//
// On hosts without nvcc the .cu file compiles via host polyfill; the same
// kernel body runs on the CPU and yields byte-equal output by construction
// because the constant table is generated from the CPU body itself.

#include "../cpp/element.hpp"
#include "../cpp/fp.hpp"
#include "../cpp/fr.hpp"
#include "../gpu/cuda/banderwagon_driver.h"
...
uint64_t lcg_state = 0x515FA1A1CAFE0001ULL;
uint8_t lcg_byte() {
...
int main() {
    std::fprintf(stdout, "=== banderwagon CPU vs CUDA byte-equality ===\n");
    int failures = 0;

    {
        ...
        int rc = banderwagon_cuda_add_batch(pairs.data(), outs.data(),
                                            (unsigned long)N);
        if (rc != 0) {
            std::fprintf(stderr, "FAIL CUDA add dispatch rc=%d\n", rc);
            return 1;
        }
        int eq = 0;
        for (int i = 0; i < N; ++i) {
...

    {
        ...
        std::vector<std::uint8_t> outs(N * 96, 0);
        int rc = banderwagon_cuda_smul_batch(pts.data(), scalars.data(),
                                             outs.data(), (unsigned long)N);
        if (rc != 0) {
            std::fprintf(stderr, "FAIL CUDA smul dispatch rc=%d\n", rc);
            return 1;
        }
        int eq = 0;
        for (int i = 0; i < N; ++i) {
...

    {
        ...
        for (int s = 0; s < 5; ++s) {
            ...
            int rc = banderwagon_cuda_msm_batch(pts_buf.data(),
                                                scalars.data(),
                                                outs.data(),
                                                (unsigned long)n,
                                                (unsigned long)M);
            if (rc != 0) {
                std::fprintf(stderr, "FAIL CUDA msm dispatch rc=%d\n", rc);
                return 1;
            }
