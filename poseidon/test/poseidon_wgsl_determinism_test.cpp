// CPU vs WGSL byte-equality determinism test for Poseidon2-BN254.
//
...
// and the WGSL host-polyfill driver poseidon2_hash2_wgsl_batch, and asserts
// byte-equal output. The polyfill mirrors the WGSL kernel byte-for-byte:
// every 64-bit op is decomposed into u32 primitives so the host result is
// indistinguishable from a real wgpu dispatch.

#include "../cpp/poseidon.hpp"
#include "../gpu/wgsl/poseidon2_driver.h"
...
int main() {
    std::fprintf(stdout,
        "=== Poseidon2-BN254 CPU vs WGSL byte-equality (gnark v0.20.1) ===\n");
    ...
    int rc = poseidon2_hash2_wgsl_batch(pairs.data(), gpu_out.data(),
                                        (unsigned long)N);
    if (rc != 0) {
