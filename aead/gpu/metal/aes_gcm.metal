...
// TLS-record-sized messages. Per-block fanout is a future kernel.
//
// Layout matches gpu/metal/aead_batch.metal (struct AeadJob; same
// keys/nonces/inputs_arena/outputs_arena packing).
//
// Constant-time S-box: Boyar-Peralta (J. Cryptol. 2010), ported byte-for-byte
// from the CPU body. No table lookups, no data-dependent branches.
// Constant-time GHASH: 128 iterations regardless of input bits.
...
struct AeadJob {
    ...
    uint32_t ct_offset;
    uint32_t tag_offset;
    uint32_t key_offset;     // i*32 (AES-256 key)
    uint32_t nonce_offset;   // i*12 (96-bit IV)
};

// AES-256: 14 rounds, expanded round-key schedule = 240 bytes (15 round keys).
constant uint AES256_ROUNDS   = 14u;
constant uint AES256_KS_BYTES = 240u;

// ---------------------------------------------------------------------------
// AES S-box -- Boyar-Peralta circuit. Byte-for-byte port of
// kinet::crypto::aead::aes::aes_sbox in cpp/aead.cpp.
// ---------------------------------------------------------------------------
...
constant uint8_t RCON[7] = { 0x01u, 0x02u, 0x04u, 0x08u, 0x10u, 0x20u, 0x40u };
...
// 60 32-bit words; each word is 4 bytes.
// ---------------------------------------------------------------------------
inline void aes256_expand_key(const device uint8_t* key, thread uint8_t* rk) {
    for (uint i = 0u; i < 32u; ++i) rk[i] = key[i];

    for (uint i = 8u; i < 60u; ++i) {
...
// ---------------------------------------------------------------------------
// AES-256 encrypt one 16-byte block (FIPS 197 §5.1). State layout is column-
// major: s[c*4 + r]. Byte-for-byte equivalent to aes::encrypt_block in
// cpp/aead.cpp.
// ---------------------------------------------------------------------------
inline void aes256_encrypt_block(thread const uint8_t* rk,
                                 thread const uint8_t* in,
                                 thread uint8_t* out) {
    uint8_t s[16];
    for (uint i = 0u; i < 16u; ++i) s[i] = (uint8_t)(in[i] ^ rk[i]);

    for (uint round = 1u; round < AES256_ROUNDS; ++round) {
        // SubBytes.
        for (uint i = 0u; i < 16u; ++i) s[i] = aes_sbox(s[i]);
        ...
        // AddRoundKey.
        for (uint i = 0u; i < 16u; ++i) s[i] ^= rk[round * 16u + i];
    }
...
// ---------------------------------------------------------------------------
inline void ghash_mul(thread uint8_t* z, thread const uint8_t* h) {
    uint8_t v[16];
    for (uint i = 0u; i < 16u; ++i) v[i] = h[i];
    uint8_t r[16];
    for (uint i = 0u; i < 16u; ++i) r[i] = 0u;

    for (uint i = 0u; i < 128u; ++i) {
        uint8_t zbit = (uint8_t)((z[i >> 3] >> (7u - (i & 7u))) & 1u);
        uint8_t mask = (uint8_t)(0u - (uint)zbit);  // 0xff if 1, else 0
        for (uint j = 0u; j < 16u; ++j) r[j] ^= (uint8_t)(v[j] & mask);

        uint8_t lsb = (uint8_t)(v[15] & 1u);
        for (uint j = 15u; j > 0u; --j) {
            v[j] = (uint8_t)((v[j] >> 1) | ((v[j-1u] & 1u) << 7));
        }
        v[0] >>= 1;
        uint8_t rmask = (uint8_t)(0u - (uint)lsb);
        v[0] ^= (uint8_t)(0xe1u & rmask);
    }

    for (uint i = 0u; i < 16u; ++i) z[i] = r[i];
}

// inc32 on the rightmost 32 bits (big-endian) of a 16-byte counter.
inline void inc32(thread uint8_t* ctr) {
    uint c = ((uint)ctr[12] << 24) | ((uint)ctr[13] << 16)
           | ((uint)ctr[14] <<  8) |  (uint)ctr[15];
    c += 1u;
    ctr[12] = (uint8_t)(c >> 24);
    ctr[13] = (uint8_t)(c >> 16);
    ctr[14] = (uint8_t)(c >>  8);
...
// Absorb a buffer (from device memory) into GHASH, padding the final partial
// block with zeros.
inline void ghash_update(thread uint8_t* y, thread const uint8_t* h,
                         const device uint8_t* arena,
                         uint off, uint len) {
    uint pos = 0u;
    while (len - pos >= 16u) {
        for (uint i = 0u; i < 16u; ++i) y[i] ^= arena[off + pos + i];
        ghash_mul(y, h);
        pos += 16u;
    }
    uint rem = len - pos;
    if (rem > 0u) {
        uint8_t buf[16];
        for (uint i = 0u; i < 16u; ++i) buf[i] = 0u;
        for (uint i = 0u; i < rem; ++i) buf[i] = arena[off + pos + i];
        for (uint i = 0u; i < 16u; ++i) y[i] ^= buf[i];
        ghash_mul(y, h);
    }
}

// Same, but absorbs from a writable arena (used after we have written
// ciphertext to outputs_arena).
inline void ghash_update_dev_w(thread uint8_t* y, thread const uint8_t* h,
                               device const uint8_t* arena,
                               uint off, uint len) {
    uint pos = 0u;
    while (len - pos >= 16u) {
        for (uint i = 0u; i < 16u; ++i) y[i] ^= arena[off + pos + i];
    ...
    uint                   gid            [[thread_position_in_grid]])
{
    ...
    const device uint8_t* iv  = nonces + job.nonce_offset;
    ...
    uint8_t Y[16];
    for (uint i = 0u; i < 16u; ++i) Y[i] = 0u;
    ghash_update(Y, H, inputs_arena, job.aad_offset, job.aad_len);
    ghash_update_dev_w(Y, H, outputs_arena, job.ct_offset, job.pt_len);
    {
        uint8_t lens[16];
        ulong la = (ulong)job.aad_len * 8ul;
        ulong lc = (ulong)job.pt_len  * 8ul;
        // BE encoding.
        for (uint i = 0u; i < 8u; ++i) lens[i]      = (uint8_t)(la >> (8u * (7u - i)));
        for (uint i = 0u; i < 8u; ++i) lens[8u + i] = (uint8_t)(lc >> (8u * (7u - i)));
        for (uint i = 0u; i < 16u; ++i) Y[i] ^= lens[i];
        ghash_mul(Y, H);
    }
