// CPU vs CUDA byte-equality cross-check for batched AEAD ciphers
// (ChaCha20-Poly1305 + AES-256-GCM).
//
// Generates 100 deterministic (key, nonce, aad, plaintext) tuples per cipher,
// encrypts on CPU and on GPU, asserts byte-identical ciphertext+tag. Plus a
// batch-size sweep M = {1, 8, 64, 256} for each cipher. Total: 200 random
// determinism cases (100 ChaCha + 100 AES-GCM) plus the sweep harness.

#include "../cpp/aead.hpp"
#include "../gpu/cuda/aead_driver_cuda.h"
...
int run_batch(Cipher cipher, size_t M, uint64_t seed_base) {
    ...
    if (rc != 0) {
        std::fprintf(stderr, "FAIL GPU dispatch cipher=%d M=%zu rc=%d\n",
                     (int)cipher, M, rc);
        return 1;
    }
    ...
    if (failures == 0) {
        const char* name = (cipher == Cipher::ChaCha)
            ? "ChaCha20-Poly1305" : "AES-256-GCM";
        std::fprintf(stdout,
            "ok    %s CUDA batch M=%zu: %zu messages byte-equal\n",
            name, M, M);
    }
    return failures;
}
...
int main() {
    if (!kinet_aead_cuda_available()) {
        std::fprintf(stdout,
            "[aead-cuda-determinism] CUDA unavailable on this host -- "
            "build-only smoke (CI runner with NVCC enforces byte-equality)\n");
        return 0;
    }

    int failures = 0;

    // Hard requirement per cipher: 100 random tuples byte-equal CPU vs CUDA.
    failures += run_batch(Cipher::ChaCha, 100, 0x1f3c2a9d4e5f6071ULL);
    failures += run_batch(Cipher::AesGcm, 100, 0xc0ffeed00dfeed00ULL);

    // Batch-size sweep, both ciphers.
    ...
    if (failures == 0) {
        std::fprintf(stdout,
            "PASS aead_cuda_determinism_test: 100 ChaCha + 100 AES-GCM "
            "+ sweep M={1,8,64,256} byte-equal CPU vs CUDA\n");
        return 0;
    }
    std::fprintf(stderr,
        "FAIL aead_cuda_determinism_test: %d mismatch(es)\n", failures);
    return 1;
}
